        ...
        if let Some(author) = self.clients.get_mut(&author_addr) {
            ...
            if diff >= MESSAGE_RATE {
                if let Ok(text) = str::from_utf8(&bytes) {
                    ...
                    if author.authed {
                        println!(
                            "INFO: Client {author_addr} sent message", // {bytes:?}
                            author_addr = Sens(author_addr)
                        );
                        println!("{text}");
                        ...
                        if text == self.token {
                            ...
                            let _ = writeln!(author.conn.as_ref(), "Welcome to the Club buddy!")
                                .map_err(|err| {
                                    eprintln!(
                                        "ERROR: could not send welcome message to {}: {}",
                                        Sens(author_addr),
                                        Sens(err)
                                    );
                                });
                            ...
                            let _ = writeln!(author.conn.as_ref(), "Invalid token! Bruh!").map_err(
                                |err| {
                                    eprintln!(
                    ...
                    if author.strike_count >= STRIKE_LIMIT {
                        ...
                        let _ =
                            writeln!(author.conn.as_ref(), "You are banned MF").map_err(|err| {
                                eprintln!(
                                    "ERROR: could not send banned message to {author_addr}: {err}",
                                    author_addr = Sens(author_addr),
                                    err = Sens(err)
                ...
                // Sending Invalid or Non-UTF-8 Data
                author.strike_count += 1;
                if author.strike_count >= STRIKE_LIMIT {
                    println!(
                        "INFO: Client {author_addr} got banned",
...
fn client(stream: Arc<TcpStream>, messages: Sender<Message>) -> Result<()> {
    ...
    messages
        ...
        .map_err(|err| {
            eprintln!(
                "ERROR: could not send message from {author_addr} to the server thread: {err}",
                author_addr = Sens(author_addr),
                err = Sens(err)
            )
        })?;
    ...
    loop {
        let n = stream.as_ref().read(&mut buffer).map_err(|err| {
            eprintln!(
                "ERROR: could not read message from {author_addr}: {err}",
                author_addr = Sens(author_addr),
                err = Sens(err)
            );
            let _ = messages
